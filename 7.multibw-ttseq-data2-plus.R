...
#read delim (plus strand)
allchr = read.delim("ttseq_noscale_cpm_fwd-data2-plus.tab")
head(allchr)
colnames(allchr)
...
conds_plus = paste(conds, "plus", sep = "_")

#change col names 
chr_sxx_names = c("chr", "start", "end", conds_plus)
chr_sxx_names
...
#annotation
ref = read.delim("plus-fwd_grch38_114_norRNA_gene-feature.bed", sep = "\t", header = F)
head(ref)
str(ref)
...
head(multibw)
write.csv(multibw, "ttseq-data2-plus-fwd.csv")
